...
\chapter{Hash Functions and Applications}
\lecture{18}{15 Mar. 10:30}{Hash-and-Mac, HMAC, Public Key Cryptography}
We can use hash functions to turn a primitive MAC that can only handle a fixed
number of bits into one that can take an arbitrary length.

\begin{theorem}
	For a cryptographic hashing function, \(H: X \to \{0, 1\}^n\), and a
	\emph{fixed length} MAC \(\Pi = (\text{Gen, Tag, Ver})\), a scheme in which a  
	message of \emph{arbitrary length}, \(m \in \{0, 1\}^*\)  is tagged can be
	achieved by simply outputting \(\text{Tag}_k(H(x))\). This scheme is a 
	\textbf{UFCMA} MAC.
\end{theorem}

\begin{proof}
	Let \(\Pi^{\prime} = (\text{Gen', Tag', Ver'})\) where Gen' and Ver' are the
	same as before, but \(Tag^{\prime}_k(m) = Tag_k(H(m)) \). We have adversaries
	\(\mathcal{A}\) against \(\Pi\) and \(\mathcal{A}^{\prime}\) against
	\(\Pi^{\prime} \), where \(\Pi^{\prime}\) handles \(m \in \{0, 1\}^*\). In the
	game, \(\Pi^{\prime} \) queries \(Tag^{\prime}_k \) multiple \(m_i \in \{0, 1\}^*\)
	and, for each \(m_i\), receives \(t_i\).
