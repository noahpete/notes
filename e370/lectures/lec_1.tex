\chapter{Pipelining}
\lecture{13}{16 Feb. 1:30}{Data Hazards and Forwarding}
\section{Data Hazards}

\begin{definition}[Data hazard]
	\emph{Data hazards} occur if one instruction uses a register value that
	hasn't been updated by a prior instruction yet.
\end{definition}

\begin{note}
	Assume the register file yields the correct value of a register when 
	read/written during the \emph{same} cycle. This is consistent with most
	processors (eg. ARM/x86), but not Project 3.
\end{note}
...
\end{corollary}
\begin{proof}
	The proof should be trivial, we left it to you.\todo{TODO}
\end{proof}
\begin{prev}
